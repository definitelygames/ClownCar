...
public class VehicleTelemetry : MonoBehaviour
	...
	void OnGUI ()
		...
		if (target != null && show)
			{
			GUI.Box(new Rect (8, 8, 600, 180), "Telemetry");
	...
	string DoTelemetry ()
		...
		text += string.Format("\n     DownForce:{0,6:0.}  Weight:{1,7:0.0}\n"+
							  "                         Mass:{2,7:0.0}\n",
							  downForce, downForce/9.81f, target.cachedRigidbody.mass);
	...
	string GetWheelTelemetry (WheelData wd, ref float suspensionForce)
		...
		string text = string.Format("{0,-20}{1}{2,5:0.} rpm ", wd.collider.gameObject.name, sleeping? "×" : ":", wd.angularVelocity * VehicleController.WToRpm);
