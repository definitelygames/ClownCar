...
public class VehicleDamage : MonoBehaviour
	{
	[Tooltip("Meshes of the vehicle to be deformed on impacts.")]
	public MeshFilter[] meshes;
	[Tooltip("Colliders of the vehicle to be deformed on impacts. This affects handling slightly.")]
	public MeshCollider[] colliders;
	[Tooltip("Transforms of the vehicle to be displaced and rotated on impacts. Include the WheelCollider transforms for impacts to greatly affect the handling of the vehicle.")]
	public Transform[] nodes;

	[Space(5)]
	[Tooltip("Minimum relative velocity between bodies at the point of contact to consider the contact an impact with deformation (m/s)")]
	public float minVelocity = 1.0f;
	[Tooltip("When an impact is detected the realtive velocity is multiplied by this factor to produce the deformations")]
	public float multiplier = 1.0f;

	[Space(5)]
	[Tooltip("An impact will affect the vertices and nodes within this distance of the impact point (m)")]
	public float damageRadius = 1.0f;
	[Tooltip("Maximum vertex and node displacement from their original positions as for the impacts (m)")]
	public float maxDisplacement = 0.5f;
	[Tooltip("Maximum random displacement of individual vertices and nodes (m). May cause visible holes in the mesh.")]
	public float maxVertexFracture = 0.1f;

	[Space(5)]
	[Tooltip("Nodes may be randomly rotated only if the impact occurs within this distance of the node (m). When nodes are WheelCollider transforms, this greatly affects the vehicle handling.")]
	public float nodeDamageRadius = 0.5f;
	[Tooltip("Maximum total rotation allowed in nodes affected by impacts (deg)")]
	public float maxNodeRotation = 14.0f;
	[Tooltip("Maximum rotation per single impact in nodes (deg)")]
	public float nodeRotationRate = 10.0f;

	[Space(5)]
	[Tooltip("Velocity at which vertices and nodes come to their original positions when the repair key is used (m/s)")]
	public float vertexRepairRate = 0.1f;

	[Tooltip("Enables the auto-repair hotkey that removes all damage from the vehicle")]
	public bool enableRepairKey = true;
	[Tooltip("A key that removes all damage from the vehicle when enableRepairKey is enabled")]
